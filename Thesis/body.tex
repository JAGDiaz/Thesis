...
Machine Learning (ML). The ability of Neural Netorks (NNs) to learn arbitrary
functions subject to constraints motivates their use to avoid the shortcomings
that these numerical methods exhibit for highly nonlinear data. Seeking new data 
representations can extend the utility of many data-driven methods and NNs
provide a new avenue of investigation in this field.
The ultimate goal of this thesis is to show that we have
...
Before we move on, a brief digression on NNs is appropriate. Ever since
...
\begin{figure}[ht]
    ...
    \begin{minipage}{.5\textwidth}
        $$\mathcal{E}$$
        \centering
        \includegraphics[width=.9\textwidth]{../Figures/Encoder.png}
    ...
    \begin{minipage}{.5\textwidth}
        $$\mathcal{D}$$
        \centering
        \includegraphics[width=.9\textwidth]{../Figures/Decoder.png}
        \begin{tabularx}{\textwidth}{>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X}
...
$\{x_i\}_{i=1}^n$ is the data that generates our histogram. In this context, a kernel
is a non-negative function that satisfies 
\begin{equation}
    \int_{-\infty}^\infty K(x)\ dx = 1 \text{ and } K(x) = K(-x),\ \forall x \in \R
\end{equation}
and can be thought of a dictating the shape of the density estimate derived.
Per Epanechnikov, 
...
%Not only is this quantity unknown, but
%there is no guarantee that $f''$ exists for an arbitrary density. 
The ISJ uses an 
...
The interesting attributes of this system are that it has three fixed points, two of which are non-linear 
