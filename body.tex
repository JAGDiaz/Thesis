...
\begin{equation}
    g(\boldsymbol{x}) = \sum_{\ell = 1}^{N_O} a_\ell \boldsymbol{\phi}_\ell
    (\boldsymbol{x}) 
\end{equation}
...
\begin{align} 
    ...
    \exp(\delta t\lambda_\ell)\boldsymbol{\phi}_\ell (\boldsymbol{x}) \\
...
$\bracks{{\bf k}_\ell}$, and ${\bf \Lambda}$ is the diagonal matrix whose elements are $\lambda_\ell
= ({\bf \Lambda})_{\ell\ell}$
...
From this we can consider ML as a tool of regression\cite{oreilly,theodoridis}. Say we have a set of training data
...
without changing the trend, so for the time being we ignore it. In the interest of trying to identify a 
...
on the initial condition\cite{strogatz:2000}. Table \ref{table:duffing params} describes the hyperparameters used for the 
...
with the other latent dimensions showing not only much larger average slopes, but as also much more 
variance. We may contrast this with the losses in Figure \ref{fig:duffing DLDMD results} for each latent 
...
should be much variability with respect to initial conditions\cite{strogatz:2000}. We would expect training on this system 
...
which is not the case for Duffing. The exact interpretation of this will be discussed later, but we've come 
...
Having presented these results, we will briefly discuss some of the consequences we can deduce from 
...
but this refocusing is something we learned in hindsight and wasn't a more obvious direction of 
...
sets of weights from previous epochs and choosing slope bounds that you'd like the averages to fall 
into. As a proof of concept, we've found that the data in the form of the weights of a model can be
used to characterize the model's evolution outside of the traditional metrics of ML and gives us a
statistical viewpoint of the seemingly random walk that these models take during training. We've also 
